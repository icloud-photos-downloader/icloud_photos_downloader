...
class SharedStreamsAlbumDict(dict):
    def __init__(
        self,
        albums_by_name: Dict[str, "SharedStreamAlbum"],
        albums_by_guid: Dict[str, "SharedStreamAlbum"],
    ):
        super().__init__(albums_by_name)
        self.albums_by_guid = albums_by_guid

    def __contains__(self, key: object) -> bool:
        return super().__contains__(key) or key in self.albums_by_guid

    def __getitem__(self, key: str) -> "SharedStreamAlbum":
        if super().__contains__(key):
            return super().__getitem__(key)
        if key in self.albums_by_guid:
            return self.albums_by_guid[key]
        raise KeyError(key)
    ...
    def albums(self) -> Dict[str, "SharedStreamAlbum"]:
        if self._albums is None:
            ...
            for album in self._fetch_albums():
                ...
                by_name[shared_album.name] = shared_album
                by_guid[shared_album.album_guid] = shared_album
            self._albums = SharedStreamsAlbumDict(by_name, by_guid)
