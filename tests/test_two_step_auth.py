...
class TwoStepAuthTestCase(TestCase):
    def test_2sa_flow_invalid_device_2fa(self):
        with vcr.use_cassette('tests/vcr_cassettes/2sa_flow_invalid_device.yml'):
            ...
            self.assertIn(
                'ERROR    Failed to verify two-factor authentication code', result.output)
    ...
    def test_2sa_flow_device_2fa(self):
        with vcr.use_cassette('tests/vcr_cassettes/2sa_flow_valid_device.yml'):
            ...
            self.assertIn(
                'INFO     Two-step/two-factor authentication is required!', result.output)
            self.assertIn(
                '  0: SMS to *******03', result.output)
            self.assertIn(
                '  1: Enter two-factor authentication code', result.output)
            self.assertIn(
                'Please choose an option: [0]: 1', result.output)
            self.assertIn('Please enter two-factor authentication code: 654321', result.output)
            self.assertIn("INFO     Great, you're all set up. The script can now be run without "
                          "user interaction until 2SA expires.", result.output)
            self.assertIn('DEBUG    Looking up all photos and videos...', result.output)
            self.assertIn('INFO     All photos have been downloaded!', result.output)
    ...
    def test_2sa_flow_sms(self):
        with vcr.use_cassette('tests/vcr_cassettes/2sa_flow_valid_sms.yml'):
            ...
            self.assertIn(
                '  1: Enter two-factor authentication code', result.output)
            self.assertIn(
                'Please choose an option: [0]: 0', result.output)
            self.assertIn(
        ...
        with vcr.use_cassette('tests/vcr_cassettes/2sa_flow_valid_sms.yml'):
            with mock.patch.object(PyiCloudService, "send_verification_code") as svc_mocked:
                ...
                self.assertIn(
                    'Please choose an option: [0]: 0', result.output)
                self.assertIn(
                    'ERROR    Failed to send two-factor authentication code', result.output)
                assert result.exit_code == 1
